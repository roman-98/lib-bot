...
const start = async () => {
    ...
    bot.on('message', async (payload) => {
        ...
        try {
            ...
            while(rows.length) {
                const chunk = rows.splice(0, BOOKS_PER_MESSAGE); // Chunk rows that came from db to N items by reducing it using splice
                const books = chunk.map(row => row.book);        // Then that chunk to array of book names using map
                const message = books.join("\n");                // Convert chunk of book names to message by joining by some logic in our case with newline symbol
                messages.push(message);                          // Push it to messages array
            }
